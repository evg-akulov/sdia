...
Фиксированный размер очереди при заполнении:
Увеличить max.poll.interval.ms: установить значение достаточно большим, например 5–15 минут (300000–900000 ms) в зависимости от худшего-case времени обработки. Это даёт больше времени между вызовами poll
Использовать max.poll.records для уменьшения объёма работы за один цикл: установить разумное маленькое значение (например 1–100) чтобы каждое извлечение давало ограниченное количество записей и обработка одного цикла занимала меньше времени
Делать частые poll() в фоне при длительной обработке: выделить отдельный поток/пулы для обработки: основной поток регулярно вызывает poll() (или вызывает poll() с коротким таймаутом), а обработка сообщений передаётся в worker-потоки. Так потребитель остаётся активным и не вызывает ребаланс
Использовать pause/resume для управления потоком: при переполнении очереди обработки можно вызывать consumer.pause(partitions) и потом resume, чтобы не накапливать новые сообщения, но при этом продолжать вызывать poll() для поддержания сессии
Увеличить session.timeout.ms при необходимости
Асинхронная обработка и коммит офсетов: используем enable.auto.commit=false и делаем коммиты после успешной обработки. В схеме с выделенными worker-потоками коммит делаем только после окончания обработки соответствующих записей
Контроль времени обработки и таймауты: добавить таймауты/контроль долгих задач; если обработка сообщения может виснуть, прерываем задачу, чтобы не держать poll заблокированным

Рекомендуемая конфигурация (примерные значения):
max.poll.interval.ms = 300000 (5 минут) — или больше при необходимости
max.poll.records = 50
...
attempt-count храним в БД в отдельном полe outbox
